...
const ShopBanner = ({
  ...
  return (
    <section className="relative bg-gradient-to-br from-[#F5EFE7] via-[#EDE0D0] to-[#E8D8C4] overflow-hidden">
      ...
      <div className="absolute right-10 bottom-0 w-44 h-[90%] rounded-t-full border border-[#9A7A46]/15 hidden md:block" />
      <div className="absolute right-24 bottom-0 w-28 h-[70%] rounded-t-full bg-[#9A7A46]/[0.04] hidden md:block" />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-14">
        ...
        <nav className="flex items-center gap-2 mb-4" aria-label="Breadcrumb">
          <Link
            to="/"
            className="text-[10px] tracking-[0.25em] uppercase text-[#9A7A46]/60 hover:text-[#9A7A46] transition-colors"
          >
            Home
          </Link>
          ...
          {categoryName && (
            <>
              <span className="text-[#9A7A46]/30 text-xs">·</span>
              <span className="text-[10px] tracking-[0.25em] uppercase text-[#9A7A46]">
                {categoryName}
              </span>
            </>
          )}
        ...
        {/* Search context pill */}
        {searchQuery && (
          <div className="inline-flex items-center gap-2 mb-4 px-3.5 py-1.5 bg-white/60 border border-[#9A7A46]/30 rounded-full backdrop-blur-sm">
            <Search size={11} className="text-[#9A7A46]" />
        ...
        <div className="flex flex-wrap items-center gap-3">
          <p className="text-sm text-[#5C3D2A] font-light italic font-serif">
            {sub}
          </p>
          {totalCount !== undefined && (
            <span className="text-[10px] tracking-[0.2em] uppercase text-[#9A7A46] bg-[#9A7A46]/10 px-2.5 py-1 rounded-sm border border-[#9A7A46]/20">
